...
module Miro
  ...
  private
    ...
    def image_magick_params
      if Miro.histogram?
        "':in[0]' -resize :resolution -colors :colors -colorspace :quantize -quantize :quantize -colors 32 -alpha remove -format %c histogram:info:"
      else
        "':in[0]' -resize :resolution -colors :colors -colorspace :quantize -quantize :quantize :out -colors 32"
      end
    end
