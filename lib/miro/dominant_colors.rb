...
module Miro
  ...
  private
    ...
    def image_magick_params
      ...
      if Miro.histogram?
        "':in[0]' -resize :resolution -type palette -depth 2 -colors 64 -channel RGBA -colorspace transparent -quantize :quantize -alpha remove -format %c histogram:info:"
      else
        "':in[0]' -resize :resolution -type palette -depth 2 -colors 64 -channel RGBA -colorspace transparent -quantize :quantize :out"
      end
    end
